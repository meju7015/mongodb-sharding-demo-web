...
class Connection
{
    ...
    protected function setMissingProperty($key, $value)
    {
        ...
        throw new ModelException(
            'Property object 를 찾을수 없습니다. 초기화가 필요합니다.',
            500,
            "objcet:{$this}, property:{$key}, value:{$value}"
        );
    }
    ...
    public function commit()
    {
        if (!$this->inTransaction()) {
            throw new ModelException(
                'commit() 이 실행되려면 transaction 이 시작되어야 합니다.',
                500
            );
        }
    ...
    public function rollBack()
    {
        if (!$this->inTransaction()) {
            throw new ModelException(
                'rollBack() 이 실행되려면 transaction 이 시작되어야 합니다.',
                500
            );
        }
